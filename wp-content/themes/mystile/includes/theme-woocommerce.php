...
function woo_wc_search_form( $form ) {
    ...
    <input type="hidden" name="post_type" value="product" />
	 ...
	 $form = 	'<form role="search" method="get" id="searchform" action="' . esc_url(home_url( '/' )) . '" >
					<div class="search-bar table-container">
						<div class="table-cell">
							<div class="input-group">
								<span class="input-group-btn"><button type="submit" class="search-btn" id="searchsubmit" value="<'. esc_attr__( 'Search', 'woothemes' ) .'">Productos</button></span>
								<input type="search" class="search search-field" placeholder="Buscar" results=5 autosave="'. esc_url(home_url( '/' )) .'" class="input-text" placeholder="'. esc_attr__( 'Search Products', 'woothemes' ) .'" value="' . get_search_query() . '" name="s" id="s" />
								<span class="glyphicon glyphicon-search glyph-right"></span>
								<input type="hidden" name="post_type" value="product" />
							</div>
						</div>
					</div>
...
/*-------------------------------------------------------------------------------------------*/
/* HOMEPAGE CUSTOM ACTIONS */
/*-------------------------------------------------------------------------------------------*/
add_action('mystile_homepage_featured', 'mystile_featured_products', 10);
add_action('mystile_homepage_recent', 'mystile_recent_products', 10);
add_action('mystile_homepage_categories', 'mystile_product_categories', 10);
...
function woocommerce_cart_link() {
	...
	<a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('View your shopping cart', 'woothemes'); ?>" class="shopping-cart">
	<img src="<?php bloginfo('template_url')?>/img/index/icons/shopping.png" alt="shopping">
	<p>
	<span class="title">Carrito</span><br>	
	<?php
	echo '<span class="items">' . sprintf(_n('%d item', '%d items', $woocommerce->cart->get_cart_contents_count(), 'woothemes'), $woocommerce->cart->get_cart_contents_count()) . ' - ' . $woocommerce->cart->get_cart_total() . '</span>';
	...
	<?php if(is_user_logged_in()) {
		echo '<p class="center logout"><a href="?page_id=30&customer-logout"">Salir</a><p>';
	} ?>
	</div>
	<!--</li>-->
	<?php
